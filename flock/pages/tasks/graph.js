...
const Graph = () => {
	...
	const [featureMap, changeFeatures] = useState({})
	...
	const getSimilarityData = (data) => {
		...
		for (let i = 0; i < data.length - 2; i++) {
			...
			let firstFeatures = Object.values(firstItem["features"])
			let secondFeatures = Object.values(secondItem["features"])
			...
			count[firstItem["name"]] = 0
			count[secondItem["name"]] = 0

			let link_ob = {}
			link_ob["source"] = firstItem["name"]
			link_ob["target"] = secondItem["name"]
			link_ob["value"] = edge_weight

			output["links"].push(link_ob)
		}

		output["links"].map((link) => {
			count[link["source"]] = count[link["source"]] + 1
			count[link["target"]] = count[link["target"]] + 1
		})
	...
	const handleChangeEvent = (e) => {
		let featureIndex = e.target.value
		featureMap[featureIndex].status = !featureMap[featureIndex].status
	...
	useEffect(() => {
		if (snapshots.length > 1) {
			if (!featureMap.length) {
				let ob = {}
				for (let i = 0; i < snapshots[1].val().length - 1; i++) {
					ob[i] = {}
					ob[i]["status"] = true
					ob[i]["text"] = snapshots[1].val()[i].text
				}
				changeFeatures(ob)
						...
						{ 
							Object.values(featureMap).map((feature) => {
								...
								return <FeatureSlider
									...
									updateFeature={ handleChangeEvent }
								/>
							})
						}
						</ul>
